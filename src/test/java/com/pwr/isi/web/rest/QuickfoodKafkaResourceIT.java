...
import com.pwr.isi.config.KafkaProperties;
...
class QuickfoodKafkaResourceIT {
    ...
    void setup() {
        ...
        QuickfoodKafkaResource kafkaResource = new QuickfoodKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/quickfood-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/quickfood-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
